...
- 新增 `CubeCompatScrollVue3.vue`：不改动 Vue2 验证组件，Vue3 外壳使用 Composition API、`defineExpose()` 和同一套 scroll utils；文档明确 Vue3 项目接入使用 `CubeCompatScrollVue3`。

### 遗留问题

- 需要逐步确认 BetterScroll 2 插件方法和 cube-ui 旧行为之间的差异。
- 需要继续补齐完整上拉触底自动化验证、下拉阈值细节和 Vue3 真实项目接入验证。
